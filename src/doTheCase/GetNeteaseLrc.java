...
public class GetNeteaseLrc {
    ...
    public static void main(String[] args) {
    	FileInputStream in;
		try {
			in = new FileInputStream("src/doTheCase/config.properties");
			pro.load(in);
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			System.out.println("未找到指定配置文件");
			e1.printStackTrace();
			System.exit(0);
		}

        String shareLink = pro.getProperty("songId");
        String songId = getSongId(shareLink);
        System.out.println("配置中歌曲ID: "+songId);
    ...
    private static void saveLrcFile(String data) throws FileNotFoundException {
        ...
        FileOutputStream fs = new FileOutputStream(new File(pro.getProperty("savePath")+File.separator+pro.getProperty("songName")+".lrc"));
        PrintStream p = new PrintStream(fs);
        p.println(data);
        p.close();
        System.out.println("歌词文件已输出到: "+pro.getProperty("savePath")+File.separator+pro.getProperty("songName")+".lrc");
    }
